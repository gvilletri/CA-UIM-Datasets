--TRUNCATE TABLE [dbo].[Optimal_Incidents]

--GO



INSERT INTO dbo.Optimal_Incidents 
...
WHERE (CAST(incident.sys_created_on as TIMESTAMP) BETWEEN DATEADD(MONTH, -12, now()) and now()) and incident.incident_state != 6 and incident.incident_state != 7
OR  ((CAST(incident.sys_created_on as TIMESTAMP) BETWEEN DATEADD(DAY, -92, now()) and now()) and incident.incident_state = 6)
OR  ((CAST(incident.sys_created_on as TIMESTAMP) BETWEEN DATEADD(DAY, -92, now()) and now()) and incident.incident_state = 7)
') as w
...
DELETE      
      FROM       [dbo].[Optimal_Incidents]
      where       [dbo].[Optimal_Incidents].DateCreated < @ArchivePeriod
